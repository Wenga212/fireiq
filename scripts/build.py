...
def parse_checkpoint_pdf(pdf_path: Path) -> list[dict]:
    """Check Point Appliance Comparison Chart — COORDINATE-BASED parser.

    The Check Point chart lays models out in COLUMNS (a header row of model
    numbers like "9100 9200 9300"), with spec values in a grid below. Plain
    text extraction flattens each row and loses which value belongs to which
    column — especially when a cell is empty or a value has a unit ("780 Mbps"),
    which shifts everything sideways. That mis-assignment is why fields came out
    empty or wrong.

    So, like the Palo Alto parser, this reads pdfplumber WORD COORDINATES:
      1. Find header rows containing >=3 model numbers; record each model's x-centre.
      2. For each spec row (matched by label text), take the value words to the
         right of the label and assign each to the column whose centre-x is nearest.
    Empty cells simply get no value assigned — no more sideways drift.

    Every value still comes straight from the uploaded PDF.
    """
    import pdfplumber

    log(f"  Reading {pdf_path.name}")

    # Footnote/reference numbers that recur as page furniture in the chart and
    # get misread as model headers. These four appear identically on many pages
    # (they're footnote markers, not appliances). Everything else that looks like
    # a model number is accepted — we no longer prefix-guess, which previously
    # rejected real models like the Spark 2500 series.
    PHANTOM_NUMBERS = {"3511", "2544", "2647", "1242"}

    def _looks_like_model(num: str) -> bool:
        n = int(num)
        # Accept any plausible Check Point appliance number, reject known footnotes.
        if num in PHANTOM_NUMBERS:
            return False
        return 1000 <= n <= 99999

    model_tok_re = re.compile(r"^\d{4,5}$")

    # Spec rows: (label regex, json_key, kind). Order matters — more specific first.
    SPEC_ROWS = [
        (re.compile(r"threat\s+prevention", re.I),              "threat",   "gbps"),
        (re.compile(r"ngfw", re.I),                             "ngfw",     "gbps"),
        (re.compile(r"\bips\b", re.I),                          "ips",      "gbps"),
        (re.compile(r"vpn", re.I),                              "vpn",      "gbps"),
        (re.compile(r"^firewall", re.I),                        "fw",       "gbps"),
        (re.compile(r"connections?\s+per\s+second", re.I),      "newSess",  "k_to_int"),
        (re.compile(r"sessions?\s+per\s+second", re.I),         "newSess",  "k_to_int"),
        (re.compile(r"concurrent\s+(?:sessions|connections)", re.I), "sessions", "millions"),
        (re.compile(r"\bssd\b|storage", re.I),                  "ssd",      "text"),
        (re.compile(r"enclosure|form\s*factor", re.I),          "ff",       "form"),
    ]
    # Label keywords that disqualify a line from being a model header
    LABEL_WORDS = ("threat","ngfw","ips","firewall","vpn","connections","concurrent",
                   "ssd","memory","physical","appliance","mbps","gbps","tbps","storage",
                   "enclosure","power","rack","dimensions","throughput","sessions","rps",
                   "cps","latency","interfaces","ports")

    found_models: dict[str, dict] = {}

    def assign_to_columns(value_words, columns):
        """Map each value word to the nearest model column by x-centre.
        Returns {model_name: 'joined value text'}."""
        buckets = {}
        for vw in value_words:
            vx = (vw["x0"] + vw["x1"]) / 2
            # nearest column centre
            best = min(columns, key=lambda c: abs(c[1] - vx))
            # only accept if reasonably close to that column (avoid stray captures)
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page_idx, page in enumerate(pdf.pages, 1):
            ...
            # Group words into visual rows by 'top' coordinate (~3px buckets)
            rows_map: dict[int, list] = {}
            for w in words:
                key = round(w["top"] / 3) * 3
                rows_map.setdefault(key, []).append(w)
            ordered_keys = sorted(rows_map)
            ordered_rows = [sorted(rows_map[k], key=lambda w: w["x0"]) for k in ordered_keys]

            current_cols = None  # [(model_name, x_centre), ...]

            for row in ordered_rows:
                row_text = " ".join(w["text"] for w in row)
                row_words_lower = [w["text"].lower() for w in row]

                # ── Is this a model-header row? (>=3 model-number tokens, no label words) ──
                model_word_objs = [w for w in row if model_tok_re.match(w["text"])
                                   and 1000 <= int(w["text"]) <= 99999]
                has_label_word = any(any(lw in wl for lw in LABEL_WORDS) for wl in row_words_lower)

                if len(model_word_objs) >= 3 and not has_label_word:
                    real = [w for w in model_word_objs if _looks_like_model(w["text"])]
                    if len(real) >= 3:
                        current_cols = [(f"CP-{w['text']}", (w["x0"]+w["x1"])/2) for w in real]
                        log(f"  Page {page_idx}: model group {[w['text'] for w in real]}")
                ...
                # ── Spec row? Match label against the row text ──
                for label_re, key, kind in SPEC_ROWS:
                    if not label_re.search(row_text):
                        continue
                    # Value words = those to the RIGHT of the first model column's
                    # left edge minus a margin (labels sit left of all columns).
                    leftmost_col_x = min(c[1] for c in current_cols)
                    cutoff = leftmost_col_x - 55
                    if kind in ("gbps","k_to_int","millions"):
                        # numeric value words (contain a digit) plus unit tokens
                        vwords = [w for w in row
                                  if (w["x0"]+w["x1"])/2 >= cutoff
                                  and (re.search(r"\d", w["text"])
                                       or re.match(r"^(Gbps|Mbps|Tbps|GB|M|K)$", w["text"], re.I))]
                    else:
                        # text rows (SSD / form factor): keep word tokens to the right
                        vwords = [w for w in row if (w["x0"]+w["x1"])/2 >= cutoff]
                    if not vwords:
                        break
                    col_vals = assign_to_columns(vwords, current_cols)
                    ...
                    for nm, raw in col_vals.items():
                        _apply_checkpoint_value(found_models[nm], key, raw, kind, row_unit)
                    break

    # Form factor fallback by series (chart's enclosure row is inconsistent)

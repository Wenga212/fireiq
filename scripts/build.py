...
def parse_paloalto_pdf(pdf_path: Path) -> list[dict]:
    ...
    Palo Alto's product-summary PDF packs up to ~14 models across a single row
    with spec values laid out in a dense matrix that pdfplumber's text extraction
    mangles (column alignment is lost). Attempting to split values positionally
    produced zero reliable extractions in testing.

    So this parser uses a CURATED, AUTHORITATIVE spec table (PA_SPECS below),
    transcribed from the official product-summary specsheet (appmix throughput
    figures). It still OPENS the PDF to detect which PA-* models are present, so
    that when Palo Alto adds a new model the build log flags it as "in PDF but not
    in curated table" — telling you to add it. This gives correct data today and a
    clear signal when the table needs updating.

    To refresh values: update PA_SPECS with figures from the latest specsheet.
    """
    import pdfplumber

    log(f"  Reading {pdf_path.name}")

    # ── Authoritative spec table (appmix throughput) ──
    # (model, fw_Gbps, threat_Gbps, vpn_Gbps_or_None, sessions_M, newSess_int, form, series)
    PA_SPECS = [
        ("PA-410",   1.4, 0.8,  0.65, 0.064,  11400, "Desktop", "PA-400"),
        ("PA-415",   1.5, 0.8,  None, 0.2,     37500, "Desktop", "PA-400"),
        ("PA-440",   1.5, 0.78, 1.1,  0.2,     37500, "Desktop", "PA-400"),
        ("PA-445",   1.5, 0.8,  1.1,  0.2,     37500, "Desktop", "PA-400"),
        ("PA-450",   2.55,1.2,  1.7,  0.2,     48000, "Desktop", "PA-400"),
        ("PA-455",   3.6, 2.3,  1.8,  0.3,     56000, "Desktop", "PA-400"),
        ("PA-460",   4.6, 3.0,  2.3,  0.4,     69000, "Desktop", "PA-400"),
        ("PA-1410",  6.8, 3.2,  4.6,  0.945,  100000, "1U", "PA-1400"),
        ("PA-1420",  9.5, 4.8,  6.5,  1.4,    140000, "1U", "PA-1400"),
        ("PA-3410", 13.0, 7.5,  9.0,  2.0,    200000, "1U", "PA-3400"),
        ("PA-3420", 18.0,10.5, 12.0,  3.0,    300000, "1U", "PA-3400"),
        ("PA-3430", 28.0,16.0, 18.0,  5.3,    400000, "1U", "PA-3400"),
        ("PA-3440", 38.0,23.0, 23.0,  6.8,    500000, "1U", "PA-3400"),
        ("PA-5410", 56.0,37.0, 32.0,  5.0,    320000, "1U", "PA-5400"),
        ("PA-5420", 70.0,47.0, 42.0,  7.0,    370000, "2U", "PA-5400"),
        ("PA-5430", 82.0,60.0, 42.0,  9.0,    380000, "2U", "PA-5400"),
        ("PA-5440", 90.0,70.0, 58.0, 20.0,    390000, "2U", "PA-5400"),
        ("PA-5445", 90.0,76.0, 64.0, 48.0,    449000, "2U", "PA-5400"),
        ("PA-5450",200.0,189.0,85.0,100.0,   3600000, "5U", "PA-5450"),
    ]
    spec_by_model = {s[0]: s for s in PA_SPECS}

    # ── Detect which PA models the PDF actually mentions ──
    pa_model_re = re.compile(r"\bPA-\d{3,4}[A-Z0-9-]*\b")
    models_in_pdf: set[str] = set()
    try:
        with pdfplumber.open(pdf_path) as pdf:
            log(f"  PDF has {len(pdf.pages)} pages")
            for page in pdf.pages:
                text = page.extract_text(x_tolerance=2) or ""
                for tok in pa_model_re.findall(text):
                    models_in_pdf.add(tok)
    except Exception as e:
        log(f"  ⚠️  Could not open PDF ({e}); using curated table only")

    # ── Flag models present in the PDF but missing from our curated table ──
    # (normalise: strip -5G / -POE / R variants down to base for the warning)
    def base(m): return re.sub(r"-(5G|POE|DC|AC)$", "", m, flags=re.I)
    known_bases = {base(m) for m in spec_by_model}
    unknown = sorted({m for m in models_in_pdf if base(m) not in known_bases
                      and re.match(r"PA-\d", m)})
    if unknown:
        log(f"  ⚠️  In PDF but not in curated table (add to PA_SPECS): {unknown}")

    # ── Emit the curated models ──
    out: list[dict] = []
    for (model, fw, threat, vpn, sess, newp, ff, series) in PA_SPECS:
        out.append({
            "vendor": "Palo Alto",
            "model":  model,
            "series": series,
            "fw":      fw,
            "ips":     threat,          # PA quotes Threat Prevention; use as IPS proxy
            "ngfw":    fw,              # App-ID throughput ~ firewall appmix
            "threat":  threat,
            "vpn":     vpn if vpn else 0.0,
            "sessions":sess,
            "newSess": newp,
            "ssl":     None,
            "iface":   "",
            ...
            "ssd":     None,
            "proc":    "",
            "source":  "paloalto.pdf (curated specs)",
        })

    log(f"  Emitted {len(out)} Palo Alto models from curated table")
    if models_in_pdf:
        log(f"  ({len(models_in_pdf)} PA tokens seen in PDF)")
    return out

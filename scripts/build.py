...
def parse_checkpoint_pdf(pdf_path: Path) -> list[dict]:
    ...
    NOT YET IMPLEMENTED. Drop the Check Point chart PDF in datasheets/,
    run scripts/inspect.py to see its structure, then implement here.
    """
    log(f"  ⚠️  Check Point parser not implemented yet — skipping {pdf_path.name}")
    return []

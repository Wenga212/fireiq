...
def parse_checkpoint_pdf(pdf_path: Path) -> list[dict]:
    ...
    # Real Check Point appliance model-number prefixes (filters footnote numbers)
    def _is_real_cp(num: str) -> bool:
        n = int(num)
        valid_prefixes = (15,16,17,18,30,36,38,39,60,62,64,66,67,69,
                          70,91,92,93,94,95,96,97,98,150,151,152,
                          160,162,166,190,191,192,195,198,230,
                          260,270,280,286,290,291,292,293)
        return (n // 100) in valid_prefixes
        ...
        for page_idx, page in enumerate(pdf.pages, 1):
            ...
            for row in ordered_rows:
                ...
                if len(model_word_objs) >= 3 and not has_label_word:
                    real = [w for w in model_word_objs if _is_real_cp(w["text"])]
                    if len(real) >= 3:

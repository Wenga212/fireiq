...
# ── Guard against shadowing the standard library ──────────────────────────
# When run as `python scripts/build.py`, Python puts the script's own folder
# (scripts/) at the FRONT of sys.path. If any file in scripts/ shares a name
# with a stdlib module (e.g. a stray inspect.py), it shadows the real one and
# breaks third-party libs (pdfplumber/bs4 call inspect.getmro). Remove the
# script directory from sys.path so only the standard library is imported.
...
def parse_cisco_1200_html(html_path: Path) -> list[dict]:
    ...
    # Use Python's built-in parser (no lxml dependency needed in CI)

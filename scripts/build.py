...
def parse_paloalto_pdf(pdf_path: Path) -> list[dict]:
    """Palo Alto Product Summary Specsheet PDF parser.

    Structure of the Palo Alto specsheet (similar to Check Point):
      - Models are in COLUMNS (header row has model names like "PA-410 PA-415 PA-440 ...")
      - Specs are in ROWS below (Firewall throughput, Threat Prevention, IPsec VPN, etc.)
      - Each page covers a different series (PA-400, PA-1400, PA-3400, PA-5400, PA-7500, etc.)
      - Throughput values often appear as "HTTP/appmix" pair (e.g. "3.5/2.9 Gbps")
        — we take the second number (appmix = realistic mixed-traffic)

    Approach: extract text from each page, find rows whose tokens look like
    PA-NNN model names, then read the spec rows that follow.
    """
    import pdfplumber

    log(f"  Reading {pdf_path.name}")

    # PA model regex — PA followed by digits, sometimes with suffixes (-5G, etc.)
    pa_model_re = re.compile(r"\bPA-(\d{3,4}[A-Z0-9-]*)\b")

    # Spec row labels: each maps to (json_key, value_parser)
    SPEC_ROWS = [
        # Throughput specs (Gbps). Use "appmix" if present (realistic), else HTTP.
        (re.compile(r"^Firewall throughput", re.I),         "fw",       "gbps_pair"),
        (re.compile(r"^Threat Prevention throughput", re.I),"threat",   "gbps_pair"),
        (re.compile(r"^IPS throughput", re.I),              "ips",      "gbps_pair"),
        (re.compile(r"^App-ID throughput", re.I),           "ngfw",     "gbps_pair"),
        (re.compile(r"^IPsec VPN throughput", re.I),        "vpn",      "gbps"),
        # Counts
        (re.compile(r"^New sessions per second", re.I),     "newSess",  "int_count"),
        (re.compile(r"^Max(?:imum)? (?:concurrent )?sessions", re.I), "sessions", "millions"),
        # Form factor and physical
        (re.compile(r"^Form factor", re.I),                 "ff",       "form"),
        (re.compile(r"^I/?O", re.I),                        "iface",    "text"),
    ]
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page_idx, page in enumerate(pdf.pages, 1):
            ...
            # Find lines that look like model header rows (3+ PA-NNN tokens)
            for header_idx, line in enumerate(lines):
                model_matches = pa_model_re.findall(line)
                if len(model_matches) < 2:
                    continue

                # Heuristic to avoid false positives: model header lines have
                # mostly PA- tokens with no spec keywords
                lower = line.lower()
                if any(kw in lower for kw in (
                    "throughput", "session", "vpn", "gbps", "mbps",
                    "table", "compare", "feature", "performance"
                )):
                    continue

                # Construct full PA-NNN names from the matches
                current_models = [f"PA-{m}" for m in model_matches]
                log(f"  Page {page_idx}: model group {current_models}")

                # Initialise records for each model in this group
                for cm in current_models:
                    if cm not in found_models:
                        found_models[cm] = {
                            "vendor": "Palo Alto",
                            "model":  cm,
                            "series": _paloalto_series(cm),
                            "fw": 0.0, "ips": 0.0, "ngfw": 0.0, "threat": 0.0,
                            "vpn": 0.0, "sessions": 0.0, "newSess": 0,
                            "ssl": None, "iface": "", "ff": "", "ssd": None,
                            "proc": "",
                ...
                # Stop when we hit the next model header or end of page.
                for spec_line in lines[header_idx + 1:]:
                    # Check if this is another model header line
                    next_models = pa_model_re.findall(spec_line)
                    next_lower = spec_line.lower()
                    is_pure_header = (len(next_models) >= 2 and
                                       not any(kw in next_lower for kw in (
                                           "throughput", "session", "vpn",
                                           "gbps", "mbps", "feature"
                                       )))
                    if is_pure_header:
                        break

                    # Try to match against known spec rows
                    for pattern, json_key, kind in SPEC_ROWS:
                        if pattern.match(spec_line):
                            values = _paloalto_extract_values(
                                spec_line, len(current_models), kind
                            )
                            for cm, raw in zip(current_models, values):
                                _apply_paloalto_value(found_models[cm], json_key, raw, kind)
                            break

    log(f"  Parsed {len(found_models)} Palo Alto models")
...
def _apply_cisco_value(model_dict: dict, key: str, raw: str, kind: str) -> None:
    ...
    if kind == "gbps":
        v = to_gbps(raw)
        # Only overwrite if we got something meaningful, and don't downgrade.
        # (Some tables list FW throughput in both Threat Defense and ASA software
        # rows — we'd rather not zero-out a previously parsed value.)
        if v > 0 and (model_dict.get(key, 0) == 0 or kind == "gbps"):
            if model_dict.get(key, 0) < v:  # take the higher value
                model_dict[key] = v
            else:
                model_dict[key] = max(model_dict[key], v)
        ...
        if m:
            ...
            unit = (m.group(2) or "").lower()
            if unit == "m":
                model_dict[key] = max(model_dict.get(key, 0), val)
            elif unit == "k":
                model_dict[key] = max(model_dict.get(key, 0), val / 1000)
            elif unit == "b":
                model_dict[key] = max(model_dict.get(key, 0), val * 1000)
            else:
                # Bare number
                if val > 1000:
                    model_dict[key] = max(model_dict.get(key, 0), val / 1_000_000)
                else:
                    model_dict[key] = max(model_dict.get(key, 0), val)
        ...
        if m:
            ...
            unit = (m.group(2) or "").lower()
            if unit == "k":
                model_dict[key] = max(model_dict.get(key, 0), int(val * 1000))
            elif unit == "m":
                model_dict[key] = max(model_dict.get(key, 0), int(val * 1_000_000))
            else:
                model_dict[key] = max(model_dict.get(key, 0), int(val))

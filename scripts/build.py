...
def parse_checkpoint_pdf(pdf_path: Path) -> list[dict]:
    """Check Point 'Quantum Security Gateways Comparison Chart' parser.

    Built against the real Feb-2026 chart structure. Key facts about this PDF:

    - Models are COLUMNS. Each page has a header row of model numbers
      (e.g. "9100 9200 9300"), with spec rows beneath.
    - The page is divided into SECTIONS. The canonical headline numbers live
      under "Enterprise Testing Conditions". Later sections must NOT be read as
      the headline figures:
         * "RFC 3511, 2544, 2647, 1242 Performance (Lab)" -- lab/marketing numbers;
           the "3511 2544 2647 1242" there are RFC standard numbers, NOT models
           (this was the source of the old phantom-model bug).
         * "HTTP/TLS Inspection Performance" -- a SECOND "Threat Prevention" row.
         * "Ideal Testing Conditions" -- marketing firewall throughput.
      So headline rows (threat/ngfw/ips/fw) are read ONLY inside the Enterprise
      section. VPN / Connections / Sessions / SSD / Enclosure are taken wherever
      they appear (they sit under different sections on different pages), each
      captured only the FIRST time per page.
    - Units vary BY PAGE: Gbps on enterprise/datacenter pages, Mbps on Spark and
      branch pages. The unit is in the row label, e.g. "Threat Prevention (Mbps)".
    - Numbers use COMMAS as thousands separators ("1,850", "190,000") which MUST be
      stripped first, else "1,850" parses as 1 (this made values ~1000x too small).
    - Footnote superscripts ("Threat Prevention (Gbps)1") sit just left of the
      first data column and are excluded by an x-position cutoff.
    ...
    SEC_ENTERPRISE = re.compile(r"enterprise\s+testing\s+conditions", re.I)
    SEC_RFC        = re.compile(r"rfc\s*3511|performance\s*\(lab\)", re.I)
    SEC_HTTP       = re.compile(r"http/tls\s+inspection", re.I)
    SEC_IDEAL      = re.compile(r"ideal\s+testing\s+conditions", re.I)
    SEC_ADDITIONAL = re.compile(r"additional\s+features", re.I)
    SEC_HARDWARE   = re.compile(r"^hardware\b|appliance\s+configurations", re.I)
    ...
    ANYWHERE_ROWS = [
        ...
        (re.compile(r"^connections\s+per\s+second", re.I), "newSess"),
        (re.compile(r"^concurrent\s+sessions", re.I),      "sessions"),
        (re.compile(r"storage\s+size|^ssd", re.I),         "ssd"),
        (re.compile(r"^enclosure", re.I),                  "ff"),
    ]

    found_models: dict[str, dict] = {}

    def label_unit_mbps(text):
        """Unit comes from the label's (Mbps)/(Gbps) token, NOT stray cell units."""
        m = re.search(r"\((mbps|gbps)\)", text, re.I)
        if m:
            return m.group(1).lower() == "mbps"
        # bracket form "[Gbps]"
        m = re.search(r"\[(mbps|gbps)\]", text, re.I)
        if m:
            return m.group(1).lower() == "mbps"
    ...
    def cell_to_gbps(raw, label_mbps):
        """Convert one cell's text to Gbps. A per-cell 'Mbps'/'Gbps' token overrides
        the label unit (handles mixed rows like '780 Mbps | 1.5 | 1.8')."""
        cleaned = raw.replace(",", "")
        nums = re.findall(r"\d+(?:\.\d+)?", cleaned)
        if not nums:
            return None
        val = float(nums[0])
        if re.search(r"mbps", raw, re.I):
            return val / 1000
        if re.search(r"gbps", raw, re.I):
            return val
        return val / 1000 if label_mbps else val
    ...
    def assign(value_words, columns, cutoff):
        buckets = {}
        for vw in value_words:
            xc = (vw["x0"] + vw["x1"]) / 2
            if xc < cutoff:
                continue
            best = min(columns, key=lambda c: abs(c[1] - xc))
            buckets.setdefault(best[0], []).append((vw["x0"], vw["text"]))
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page_idx, page in enumerate(pdf.pages, 1):
            ...
            for w in words:
                k = round(w["top"] / 3) * 3
                rows_map.setdefault(k, []).append(w)
            ordered = [sorted(rows_map[k], key=lambda w: w["x0"]) for k in sorted(rows_map)]

            current_cols = None
            section = None

            for ridx, row in enumerate(ordered):
                row_text = " ".join(w["text"] for w in row)
                low = row_text.lower()

                def values_for(this_row):
                    """Numeric value words on this row (plus standalone unit tokens
                    like 'Mbps'/'Gbps' so mixed-unit cells keep their unit); if none,
                    peek the row directly above (labels sometimes sit just below
                    their values due to text-wrap, e.g. page 4 IPS row)."""
                    def keep(w):
                        return (re.search(r"\d", w["text"])
                                or re.match(r"^(Mbps|Gbps|Tbps)$", w["text"], re.I))
                    vw = [w for w in this_row if keep(w)]
                    # require at least one digit-bearing word to count as a value row
                    if any(re.search(r"\d", w["text"]) for w in vw):
                        return vw
                    if ridx > 0:
                        prev = ordered[ridx-1]
                        prev_alpha = [w for w in prev if re.search(r"[A-Za-z]", w["text"])
                                      and not re.match(r"^(Mbps|Gbps|Tbps)$", w["text"], re.I)]
                        prev_nums = [w for w in prev if re.search(r"\d", w["text"])]
                        if prev_nums and len(prev_alpha) == 0:
                            return [w for w in prev if keep(w)]
                    return []

                # Model-header row?
                mtoks = [w for w in row if model_tok_re.match(w["text"])
                         and 1000 <= int(w["text"]) <= 99999]
                is_rfc_line = "rfc" in low or "performance" in low
                alpha_words = [w for w in row if re.search(r"[A-Za-z]", w["text"])]
                if len(mtoks) >= 2 and not is_rfc_line and len(alpha_words) <= 1:
                    current_cols = [(f"CP-{w['text']}", (w["x0"]+w["x1"])/2) for w in mtoks]
                    section = None
                    log(f"  Page {page_idx}: model group {[w['text'] for w in mtoks]}")
                    for (nm, _) in current_cols:
                        if nm not in found_models:
                            found_models[nm] = {
                                "vendor":"Check Point","model":nm,
                                "series":_checkpoint_series(nm),
                                "fw":0.0,"ips":0.0,"ngfw":0.0,"threat":0.0,
                                "vpn":0.0,"sessions":0.0,"newSess":0,
                ...
                # Section transitions
                if SEC_ENTERPRISE.search(row_text): section = "enterprise"; continue
                if SEC_RFC.search(row_text):         section = "rfc";        continue
                if SEC_HTTP.search(row_text):        section = "http";       continue
                if SEC_IDEAL.search(row_text):       section = "ideal";      continue
                if SEC_ADDITIONAL.search(row_text):  section = "additional"; continue
                if SEC_HARDWARE.search(row_text):    section = "hardware";   continue

                leftmost = min(c[1] for c in current_cols)
                cutoff = leftmost - 60
                ...
                if section == "enterprise":
                    ...
                    for rx, key in ENT_ROWS:
                        if rx.search(row_text):
                            label_mbps = label_unit_mbps(row_text)
                            vwords = values_for(row)
                            col_vals = assign(vwords, current_cols, cutoff)
                            for nm, raw in col_vals.items():
                                g = cell_to_gbps(raw, label_mbps)
                                if g is not None and g > 0:
                                    found_models[nm][key] = round(g, 4)
                            matched = True
                            break
                    if matched:
                        continue
                ...
                # Anywhere rows
                for rx, key in ANYWHERE_ROWS:
                    if not rx.search(row_text):
                        continue
                    if key == "vpn":
                        if found_models[current_cols[0][0]].get("vpn",0) > 0: break
                        label_mbps = label_unit_mbps(row_text)
                        vwords = values_for(row)
                        col_vals = assign(vwords, current_cols, cutoff)
                        for nm, raw in col_vals.items():
                            g = cell_to_gbps(raw, label_mbps)
                            if g is not None and g > 0:
                                found_models[nm]["vpn"] = round(g, 4)
                        break
                    if key == "newSess":
                        if found_models[current_cols[0][0]].get("newSess",0) > 0: break
                        in_k = "(k)" in low
                        vwords = [w for w in row if re.search(r"\d", w["text"])]
                        col_vals = assign(vwords, current_cols, cutoff)
                        for nm, raw in col_vals.items():
                            cleaned = raw.replace(",","")
                            ...
                            if in_k and val < 10000:
                                val *= 1000
                            found_models[nm]["newSess"] = int(val)
                        break
                    if key == "sessions":
                        if found_models[current_cols[0][0]].get("sessions",0) > 0: break
                        vwords = [w for w in row if re.search(r"\d", w["text"])]
                        col_vals = assign(vwords, current_cols, cutoff)
                        for nm, raw in col_vals.items():
                            cleaned = raw.replace(",","")
                            ...
                            if mvals:
                                found_models[nm]["sessions"] = max(float(x) for x in mvals)
                            else:
                                nums = re.findall(r"\d+(?:\.\d+)?", cleaned)
                                if nums:
                                    ...
                                    if val > 10000:
                                        val = val/1_000_000
                                    found_models[nm]["sessions"] = round(val,3)
                        break
                    if key == "ssd":
                        vwords = [w for w in row if (w["x0"]+w["x1"])/2 >= cutoff]
                        col_vals = assign(vwords, current_cols, cutoff)
                        for nm, raw in col_vals.items():
                            mm = re.search(r"\d+\s*[GMT]B", raw, re.I)
                            if mm and not found_models[nm].get("ssd"):
                                found_models[nm]["ssd"] = mm.group().replace(" ","")
                        break
                    if key == "ff":
                        vwords = [w for w in row if (w["x0"]+w["x1"])/2 >= cutoff]
                        col_vals = assign(vwords, current_cols, cutoff)
                        for nm, raw in col_vals.items():
                            if raw and not found_models[nm].get("ff"):
                                found_models[nm]["ff"] = classify_form_factor(raw)
                        break

    for m in found_models.values():
        if not m.get("ff"):

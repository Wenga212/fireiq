...
def parse_checkpoint_pdf(pdf_path: Path) -> list[dict]:
    ...
    SPEC_ROWS = [
        # Throughput specs (Gbps) — broadened to catch label variants
        (re.compile(r"Threat Prevention", re.I),                "threat",   "gbps"),
        (re.compile(r"NGFW Throughput", re.I),                  "ngfw",     "gbps"),
        (re.compile(r"\bIPS Throughput\b", re.I),               "ips",      "gbps"),
        (re.compile(r"^Firewall(?:\s+Throughput)?\b", re.I),    "fw",       "gbps"),
        (re.compile(r"VPN(?:\s+AES[-\s]?256)?\s+Throughput", re.I), "vpn",  "gbps"),
        (re.compile(r"\bVPN Throughput\b", re.I),               "vpn",      "gbps"),
        # Counts — catch "Connections Per Second", "Sessions Per Second",
        # "Concurrent Sessions", "Concurrent Connections", "Maximum Concurrent..."
        (re.compile(r"Connections?\s+Per\s+Second", re.I),      "newSess",  "k_to_int"),
        (re.compile(r"Sessions?\s+Per\s+Second", re.I),         "newSess",  "k_to_int"),
        (re.compile(r"(?:Maximum\s+)?Concurrent\s+(?:Sessions|Connections)", re.I), "sessions", "millions"),
        # Physical
        (re.compile(r"SSD", re.I),                              "ssd",      "text"),
        (re.compile(r"(?:Physical\s+Enclosure|Form\s+Factor|Enclosure)", re.I), "ff", "form"),
    ]
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page_idx, page in enumerate(pdf.pages, 1):
            ...
            for header_idx, line in enumerate(lines):
                ...
                # Filter out PHANTOM models — footnote/figure reference numbers that
                # get misread as a model row. The known bogus set ['3511','2544',
                # '2647','1242'] repeats across many pages and isn't a real lineup.
                # Real Check Point appliances follow recognised series prefixes.
                def _is_real_cp(num: str) -> bool:
                    n = int(num)
                    # Known current/recent Check Point series ranges:
                    #  1500/1600/1700/1800 (branch), 3000/3600/3800, 6000/6200-6900,
                    #  7000, 9100-9800 (Quantum Force), 15000/16000, 19000, 23000,
                    #  26000/27000, 28000/28600, 29000. Also Maestro 3000/3900 series.
                    ...
                    # Check 2-digit and 3-digit prefixes
                    p2 = n // 100   # e.g. 6200 -> 62
                    p3 = n // 100   # for 5-digit handled below
                    if n >= 10000:
                        p = n // 100   # 19100 -> 191, 16200 -> 162
                        return p in valid_prefixes
                    else:
                        p = n // 100   # 6200 -> 62, 3600 -> 36
                        return p in valid_prefixes
                real_models = [m for m in models_in_line if _is_real_cp(m)]
                # If filtering removed everything or left too few, this wasn't a
                # real model header — skip it.
                if len(real_models) < 3:
                    continue
                models_in_line = real_models
    ...
    # Form factor fallback by series (the chart's enclosure row is inconsistent;
    # fill any blanks from the known chassis per Check Point series).
    for m in found_models.values():
        if not m.get("ff"):
            n = int(re.search(r"\d+", m["model"]).group())
            if n < 2000:    m["ff"] = "Desktop"        # 1500/1600/1700/1800 branch
            elif n < 7000:  m["ff"] = "1U"             # 3000/6000 series
            elif n < 16000: m["ff"] = "1U"             # 7000/9000 series
            elif n < 26000: m["ff"] = "2U"             # 16000/19000 series
            else:           m["ff"] = "2U"             # 26000/28000/29000 series
